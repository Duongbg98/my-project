...
public class AlgorithmScheduling {
    ...
    public boolean runAlgorithm() throws InconsistentEvidenceException, IOException {
        ...
        for (int indexStory = 0; indexStory < numUserStory; indexStory++) {
            ...
            // Khởi tạo thuật toán: lấy ra mảng độ ưu tiên của các task
            // int[][] arrPrecedence = this.getPrecedence(tasks);
            for (int i = 0; i < numTask; i++) {

                // tìm task có thời gian dài nhất để lập lịch
                ...
                if (task.getID() != 0) {
                    ...
                    if (resource != null && (resource.getLoaded() + task.getTime() < this.cap)) {
                        ...
                        if (resource == null) {
                            ...
                            System.out.println("Vượt length");
                        }
                        scheduled = false;
                        return false;
                    }
     ...
     * @throws IOException
     */
    private Resource findResource(Task task, List<Task> listTask) throws InconsistentEvidenceException, IOException {
        ...
        // tính thời gian bắt đầu cho task từ task tiên quyết
        Task par = findTaskById(task.getPrecedence(), listTask);
        int timeStart = 0;
        if (par != null)
            timeStart = par.getStartTime() + par.getTime();
        int index = this.findMax(prob, loadeds, timeStart);
     ...
     * @return
     */
    private int findMax(double prob[], int loaded[], int startTime) {
